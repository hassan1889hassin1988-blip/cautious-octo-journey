...
local WindUI = loadstring(game:HttpGet("https://raw.githubusercontent.com/Footagesus/WindUI/beta/dist/main.lua"))()
...
CommunityTab:Button({
    ...
    Icon = "users",
    Callback = function()
        setclipboard("https://discord.gg/7PH3f57Q2U")
        WindUI:Notify({
            Title = "Discord",
...
CommunityTab:Button({
    Title = "Join Our Friend's Discord Server",
    Icon = "users",
    Callback = function()
        setclipboard("https://discord.gg/PNWUGnsRQZ")
        WindUI:Notify({
            Title = "Discord",
            Content = "Invite copied to clipboard!",
            Duration = 3,
...
local ToonsSection = AutomationTab:Section({
    Title = "Toons",
    Icon = "sparkles",
    
    IconColor = Color3.fromHex("#FDE54E"),
    Box = true,
})
-- Variables
local Players = game:GetService("Players")
...
local elevatorModeEnabled = false
local meOnlyModeEnabled = false
local grabDistance = 75
...
local function getCooldown()
    if not localChar then return 1 end
    local abilities = localChar:FindFirstChild("Abilities")
    local ability1 = abilities and abilities:FindFirstChild("Ability1")
    local cooldown = ability1 and ability1:FindFirstChild("CurrentCooldown")
    return cooldown and cooldown.Value or 0
end

local function getMyElevatorState()
    local myChar = workspace.InGamePlayers:FindFirstChild(localPlayer.Name)
    if not myChar then return false end
    local stats = myChar:FindFirstChild("Stats")
    local inElevator = stats and stats:FindFirstChild("InElevator")
    return inElevator and inElevator.Value == true
end

local function isPanicActive()
    local panic = workspace.Info:FindFirstChild("Panic")
    return panic and panic.Value == true
end

local function isActive()
    if not getMyElevatorState() then return false end
    if not isPanicActive() then return false end
    if not elevatorModeEnabled and not meOnlyModeEnabled then return false end
    return true
end
...
ToonsSection:Toggle({
  Tags = {
    {
    Title = "Goob",
    Color = Color3.fromHex("#ded03a")
  },
},
    Title = "Auto Grab",
    Desc = "Grabs furthest player not in elevator when everyone including you are inside the elevator",
    Callback = function(state)
        elevatorModeEnabled = state
        if not state then lastTarget = nil end
    end,
})
...
ToonsSection:Toggle({
  Tags = {
    ...
    Title = "Auto Grab (only you in elevator",
    Desc = "ignores other players and grab the farthest player when you inside the elevator ",
    Callback = function(state)
        meOnlyModeEnabled = state
        if not state then lastTarget = nil end
    end,
})
...
ToonsSection:Slider({
  Tags = {
    ...
    Title = "Grab Distance",
    Desc = "Grab when target is within this many studs",
    Step = 1,
    Value = {
        Min = 10,
        Max = 500,
...
RunService.Heartbeat:Connect(function()
    ...
    if elevatorModeEnabled and not meOnlyModeEnabled then
        local outsideTarget = nil
        local outsideDist = -1

        for _, char in pairs(workspace.InGamePlayers:GetChildren()) do
            if char == localChar then continue end
            local stats = char:FindFirstChild("Stats")
            local inElevator = stats and stats:FindFirstChild("InElevator")
            if not inElevator then continue end

            if inElevator.Value == false then
                local root = char:FindFirstChild("HumanoidRootPart")
                if root then
                    local dist = (root.Position - hrp.Position).Magnitude
                    if dist > outsideDist then
                        outsideDist = dist
                        outsideTarget = char
                    end
                end
        ...
        if outsideTarget and outsideDist <= grabDistance then
            lastTarget = outsideTarget
        else
            lastTarget = nil
        end

    elseif meOnlyModeEnabled then
        ...
        if furthestTarget and furthestDist <= grabDistance then
            lastTarget = furthestTarget
        else
            lastTarget = nil
        end
    end
...
RunService.Heartbeat:Connect(function(dt)
    ...
    if getCooldown() > 0 then return end

    local abilityEvent = RS.Events:FindFirstChild("AbilityEvent")
    if not abilityEvent then return end

    pcall(function()
        abilityEvent:InvokeServer(localChar, localChar.PrimaryPart.CFrame, lastTarget, nil)
    end)
end)
