...
local Window = WindUI:CreateWindow({
    Title = "TZ HUB ||Nights of Damnation",
    Folder = "TZHub",
...
local function getKillerName()
    local success, name = pcall(function()
        return LocalPlayer.PlayerGui
            .KillerIntro.MainFrame.Main.Player.PlayerName.Text
    end)
...
local function highlightPlayer(player)
    ...
    if player.Character:FindFirstChild("KillerHighlight") then return end
...
local function clearHighlights()
    for _, player in ipairs(Players:GetPlayers()) do
        if player.Character then
            local h = player.Character:FindFirstChild("KillerHighlight")
            if h then
                h:Destroy()
            end
        end
...
task.spawn(function()
    while true do
        ...
        if killerName and killerName ~= "" then
            for _, player in ipairs(Players:GetPlayers()) do
                if player.Name == killerName or player.DisplayName == killerName then
...
local ObjectivesEnabled = false

local function highlightObjective(obj)
    if obj:FindFirstChild("ObjectiveHighlight") then return end

    local h = Instance.new("Highlight")
    h.Name = "ObjectiveHighlight"
...
local function clearObjectives()
    local folder = workspace:FindFirstChild("ActiveMap")
    if not folder then return end

    local objectives = folder:FindFirstChild("Objectives")
    if not objectives then return end

    for _, obj in ipairs(objectives:GetDescendants()) do
        local h = obj:FindFirstChild("ObjectiveHighlight")
        if h then
            h:Destroy()
        end
    end
...
task.spawn(function()
    while true do
        ...
        local objectives = folder:FindFirstChild("Objectives")
        if not objectives then continue end

        for _, obj in ipairs(objectives:GetDescendants()) do
            if obj:IsA("BasePart") or obj:IsA("Model") then
                highlightObjective(obj)
            end
        end
    end
end)
...
-- =========================
-- WINDUI TOGGLE (ESP TAB)
-- =========================

local ObjectivesToggle = espTab:Toggle({
    Title = "Objectives ESP",
    Desc = "Highlights all objectives",
    Flag = "Highlights_toggle",
    Icon = "",
    Value = false,
    Callback = function(state)
        ObjectivesEnabled = state
...
local CatchToggle = AutomationTab:Toggle({
    Title = "Auto machine",
    Desc = "",
    Icon = "hand",
    Value = false,
